...
var map = {
  ...
  post: function post() {
    ...
    cc.set_monkey("speed", default_speed * 1.0); // cc.set_monkey("cameraDownAngle", 50.0 * Math.PI / 180);
    ...
    switch (this.section_id) {
      ...
      case 1:
        if (PZ < -18) {
          ...
          setTimeout(function () {
            player1.physicsImpostor = new BABYLON.PhysicsImpostor(player1, BABYLON.PhysicsImpostor.BoxImpostor, {
              mass: 0.0,
              restitution: 1.0,
              friction: 0.5
            }, scene);
          ...
          tail_vertexData.positions = [-1, 0, 1, 0, 0, 3, 1, 0, 1];
          ...
          wings_vertexData.positions = [1, 0, -1, 3, 0, 0, 1, 0, 1, -1, 0, -1, -3, 0, 0, -1, 0, 1];
          ...
          arrow_vertexData.positions = [-1, 0, -1, // Adjusted to point towards negative z-axis
          ...
          arrow_vertexData1.positions = [-1, 0, -1, // Adjusted to point towards negative z-axis
          0, 0, -3, // Adjusted to point towards negative z-axis
          1, 0, -1 // Adjusted to point towards negative z-axis
          ];
  ...
  physics_update: function physics_update() {
    ...
    var isJumpEnabled = flyjump.can_jump;
  ...
  collision_check: function collision_check() {
    if (player1.position.y < -20) {
      change_state.die('Fell To Death');
    }

    if (player1.position.y > 80) {
      change_state.die('Left The Orbit');
    }

    this.checkConeCollision();
    this.checkEndingCollision();
  },
  checkConeCollision: function checkConeCollision() {
  ...
  checkEndingCollision: function checkEndingCollision() {
    if (score < 10) return;

    for (var i = 0; i < maker.ending_count; i++) {
      var ending = endings[i];

      if (this.are_touching(player1, ending, 1.2)) {
        // previously 1.0
        change_state.win();
        break;
      }
  ...
  player_move: function player_move() {
    ...
    if (player1 !== null) {
      if (player.position.z > -100 || player.position.z < -158) {
        ...
        player1.position.x = -player.position.x;
        player1.position.y = player.position.y;
        var positionAdjustment = window.tsTriggers.getPositionAdjustment();
    ...
    var rotation_offsetted = rotation + cameraRightAngle;
  ...
  shouldSpin: function shouldSpin() {
    if (speed === default_speed) return true;
    if (speed === 0.2) return true;
    return false;
  },
  are_touching: function are_touching(a, b, r) {
  ...
  run: function run() {
    var full_reset = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    ...
    for (var _i = 0; _i < maker.platform_count; _i++) {
      var _mesh_name = "P" + _i;

      var _mesh = scene.getMeshByName(_mesh_name);

      _mesh.dispose(true);
    }

    for (var _i2 = 0; _i2 < maker.cone_count; _i2++) {
      var _mesh_name2 = "C" + _i2;

      var _mesh2 = scene.getMeshByName(_mesh_name2);

      _mesh2.dispose(true);
    }

    for (var _i3 = 0; _i3 < maker.cylinder_count; _i3++) {
      var _mesh_name3 = "Y" + _i3;

      var _mesh3 = scene.getMeshByName(_mesh_name3);

      _mesh3.dispose(true);
    }

    for (var _i4 = 0; _i4 < maker.sphere_count; _i4++) {
      var _mesh_name4 = "S" + _i4;

      var _mesh4 = scene.getMeshByName(_mesh_name4);
